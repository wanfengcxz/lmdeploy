...
python3 profile_generation2.py ${model_path} --backend pytorch --tp ${tp} --concurrency ${max_batch_size} --cache-max-entry-count ${cache_max_entry_count} --prompt-tokens 256 --completion-tokens 128 --test-round 3 --warmup-round 1 --csv ${result_file}
